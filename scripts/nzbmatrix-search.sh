...
#    echo "$SPLAT"
    ...
    echo -e "Link: http://`$CAT $TMP/nzbsearch.txt | grep "LINK:" | sed 's/^LINK://' | awk '{print "http://"$0}' | head -2 | tail -1 | tr -d ";"`"
    ...
    echo -e "Link: http://`$CAT $TMP/nzbsearch.txt | grep "LINK:" | sed 's/^LINK://' | awk '{print "http://"$0}' | tail -1 | tr -d ";"`"
